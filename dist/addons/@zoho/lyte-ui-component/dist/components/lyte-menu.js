...
_observedAttributes :["ltPropContent","ltPropId","ltPropClass","ltPropQuery","ltPropEvent","ltPropYield","ltPropUserValue","ltPropSystemValue","ltPropCallout","ltPropPosition","ltPropDescription","ltPropTabIndex","ltPropFreeze","ltPropShow","ltPropWidth","ltPropHeight","ltPropQueryClass","ltPropBoundary","ltPropScope","ltPropPreventInsideClick","ltPropAnimate","ltPropSetCss","ltPropWrapperClass","ltPropWrapperId","ltPropBindToBody","ltPropAria","ltPropAriaAttributes","ltPropOffset","ltPropHeightResetOnScroll","ltPropPreventKeyEvents","ltPropForceClick","eventListeners","pos","parIndex","lyteUnbound","randomId"],
        ...
        mousemove: function (evt, _this) {
            ...
            if (node && node.tagName == 'LYTE-MENU-ITEM') {
                ...
                if( this.data.ltPropAria ){
                    $L( this.$node.element ).attr( 'aria-activedescendant', node.id );
                    $L( this.childComp ).attr( 'aria-activedescendant', node.id );
                }
            }
        }
    },
    ...
    open_submenu : function( evt ){
        ...
        if( !child_opened && _this.data.ltPropAria ){
            ...
            if( !selected ){
                selected = box.getElementsByTagName( 'lyte-menu-item' )[ 0 ];
                if( selected ){
    ...
    is_input_element : function( target, keyCode ){
    ...
    ___closestFind: function (path, query) {
    ...
    data: function () {
        return {
            ...
            */
             ...
             * @version 3.1.0
             */
            ltPropAriaAttributes: Lyte.attr('object', { default: { role: "menu" }, watch : true }),
            /**
            ...
            ltPropPreventKeyEvents : Lyte.attr( 'boolean', { default : false } ),

            /**
             * @experimental ltPropForceClick
             */

            ltPropForceClick : Lyte.attr( 'boolean', { default : false } ),
            ...
            randomId : Lyte.attr( 'string', { default : "lyteMenu_" + parseInt( Math.random() * 1e6 ) } )
        }
    },
    ...
    optionSelect: function (event) {
        ...
        if( nodeName && $L( nodeName ).hasClass( 'lyteMenuItemDisabled' ) ){
            return;
        }

        if( !nodeName || ( nodeName.__sub_open && !nodeName.__force_callback ) ){
            return;
        }
    ...
    */
    ...
    traverseList: function (event) {
        ...
        if (!cursel || !cursel.offsetParent) {
            if (kc != 35) {
                ...
                fn( $L('lyte-menu-item:not(.lyteSearchHidden)', this.childComp).eq(-1).addClass('lyteMenuSelection').get( 0 ) );
            }
        }
    ...
    ariaObs: function ( arg ) {
        ...
        if( data.ltPropAria && childComp ){
            _lyteUiUtils.setAttribute( childComp, data.ltPropAriaAttributes || {}, ( arg || { oldValue : {} } ).oldValue );
        }
    ...
    single_obs : function( arg ){
        ...
        if( data.ltPropAria  && childComp ){
            ...
            _lyteUiUtils.setAttribute( childComp.get( 0 ), obj, {} );
        }
...
if (!_lyteUiUtils.registeredCustomElements['lyte-menu-item']) {
    ...
    Lyte.createCustomElement("lyte-menu-item", {
        ...
        check_disable : function( value ){
            ...
            this.classList[ fn ]( "lyteMenuItemDisabled" );
        },  
        ...
        connectedCallback: function () {
            ...
            disabled = this.getAttribute( 'lt-prop-disabled' );
            ...
            if (closestBody && closestBody.parent) {
                ...
                if (aria) {
                    ...
                    if( !this.id ){
                        this.id = "lyteMenuItem_" + parseInt( Math.random() * 1e6 );
                    }
                }
            }

            disabled == "true" && this.check_disable( disabled );
        },
