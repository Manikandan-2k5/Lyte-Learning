...
Object.defineProperties($Record.prototype,{
	...
	deleteRecord : {
		value : function value(delayPers){
			var model = this.model, record = this.record, 
	...
	persist:{
		value: function(obj){
			var mName = this.model._name, rec = this.record, partialObj = {obj:new Map()}, type, dirty;
			if(this.isNew){
				type = "create";
	...
	clone:{
		value : function(){
			var parent = this.record , data = this.record.$.toJSON({type : "isClone"});
...
var store = {
	...
	$:{ 
		...
		extendService : function(services, type, name, self, scope, isGlobal, data){
			Lyte.extendService({services: services, type:type, name:name, ins: self, isGlobal:isGlobal, data:data, callback : function(serv, key, name){
		...
		setData:function(self, attr, value, opts, redoObj, ignoreChange,inherit){
			var toEmit = {emit : false, attr : [], oldRec : {}};
		...
		setValue:function(self,attr,value,opts,toEmit,ignoreChange,deepChange){
		...
		toChildJSON: function(model,reltype,record,relKey,parent,recmp,addNotDefinedFields){
								...
								if(mdlf[v].relType =="hasMany" && record[i][mdlf[v].relKey]){
									for (var v1=0; v1<record[i][mdlf[v].relKey].length; v1++){
										...
										if(!recmp.get(record[i][mdlf[v].relKey][v1])){
											...
											record[i][mdlf[v].relKey][v1]=(typeof pval == "object")?pval:{[store.modelFor(mdlf[v].relatedTo)._pK]:pval};
										}
									}
								}
									...
									if(!recmp.get(record[i][mdlf[v].relKey])){
										...
										record[i][mdlf[v].relKey]=(typeof pval == "object")?pval:{[store.modelFor(mdlf[v].relatedTo)._pK]:pval}
									}
								}
		...
		insertIntoStore:function(modelName,data,saveParent,stack,partialObj,temp,checkRelData,clone,index){
			var ret;
			if(Array.isArray(data)){
				ret = [];
				for(var i=0; i<data.length; i++){
		...
		removeFromStore:function(model,keys,fromStore,ignorePartial,delayPer,onlyRem,partOnlyRem){
		...
		validateAndPush:function(model,data,partialObj,clone){
			if(!model.rel){
				model.rel = {};
			}
		...
		validateAndMerge:function(model,data,partialObj,mergeError){
            if(!model.rel){
                model.rel  = {};
            }
		...
		mergeData:function(record,data,partialObj,mergeError){
            if(!record || !data){
                return;
            }
		...
		validateJSON:function(model,data,keys,toValidate,partialObj,clone){
		...
		establishLink:function(forward,backward,data,relatedRecord,index,ignorePartial, isDuplicate, partRemoveOnly){
			...
			var fRelKey = forward.relKey, isPoly = forward.opts && forward.opts.polymorphic, type = isPoly ? relatedRecord._type : undefined, ignoreRel = {};
			if(forward.relType == "belongsTo"){
		...
		PayloadSerialize : function(field,payload,record,key,model,partial){
			var getPayloadData=typeof record.$.pK =="object" ? record.$.pK:{[record.$.model._pK]:record.$.pK};
			var mdl = model;
...
Object.defineProperties(store,{
	...
	peekRecord:{	
		value : function value(modelName, pKey, isDeleted){
			var model = this.modelFor(modelName), modelless = store.$.getFromCB("adapter",modelName,"modelless"), _defless;
			model = modelless ? modelName : model;
			if( !model ){
				Lyte.warn("LD02","Model ",modelName);
				return;
