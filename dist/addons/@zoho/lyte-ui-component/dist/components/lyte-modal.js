...
_template:"<template tag-name=\"lyte-modal\"> <template is=\"if\" value=\"{{expHandlers(ltPropBindToBody,'&amp;&amp;',expHandlers(ltPropReRenderModal,'!'))}}\"><template case=\"true\"> <lyte-wormhole case=\"true\" style=\"{{if(ltPropShowCopy,'visibility:visible','visibility:hidden')}}\" on-before-append=\"{{method(&quot;beforeWormholeAppend&quot;)}}\" lt-prop-show=\"{{ltPropShow}}\"> <template is=\"registerYield\" yield-name=\"lyte-content\"> <div class=\"modalWrapper {{ltPropWrapperClass}}\"> <div class=\"lyteModal\"> <template is=\"if\" value=\"{{ltPropShowCloseButton}}\"> <template case=\"true\"><span class=\"lyteModalClose\" tabindex=\"0\" __click=\"{{action('close')}}\"></span></template> </template> <lyte-yield yield-name=\"modal\"></lyte-yield> </div> <template is=\"if\" value=\"{{ltPropFreeze}}\"> <template case=\"true\"><lyte-modal-freeze></lyte-modal-freeze></template> </template> </div> </template> </lyte-wormhole> </template></template> </template>",
...
_observedAttributes :["ltPropShow","ltPropFreeze","ltPropShowCloseButton","ltPropCloseOnEscape","ltPropTransition","ltPropOffset","ltPropDimmer","ltPropDraggable","ltPropAllowMultiple","ltPropScrollable","ltPropMaxHeight","ltPropMaxWidth","ltPropWidth","ltPropHeight","ltPropWrapperClass","ltPropBindToBody","ltPropShowCopy","ltPropReRenderModal","ltPropOverlayClose","ltPropAria","ltPropAriaAttributes","ltPropPreventFocus","ltPropSetContentHeight","ltPropCloseDuration","ltPropOverlapModal","ltPropIgnoreInlineDirection","ltPropAllowContainment","ltPropDependentModalId","ltPropParentModalId","first","resizeCalled","initializedPosition","prevHeight","returnedFalse","prevModalHeight","prevModalWidth","calculateHW","checkAria","beforeDragPosition","resetTriggered"],
    ...
    data: function(){
        return {
            ...
            "ltPropDependentModalId": Lyte.attr('string', {default: ''}),
            ...
            "beforeDragPosition" : Lyte.attr('object' , {default : {xPos:0,yPos:0}}),
            "resetTriggered" : Lyte.attr('boolean' , {default : false})
    ...
    handleDrag : function(e){
        ...
        var compStyle = getComputedStyle(drag);
        if(!LytePopup.node){
            return;
        }
        var curleft = 0
        ...
        if(LytePopup.node.closest('lyte-wormhole')._callee.component.getData('ltPropTransition').animation == "fadeIn"){
            curleft = (mouseOffset.x-LytePopup.xPos)
            curtop = (mouseOffset.y-LytePopup.yPos)
          ...
          if((LytePopup.node.closest('lyte-wormhole')._callee.component.getData('ltPropTransition').animation == "zoom") && !matrix){
            matrix = {}
            matrix.x = 0;
            matrix.y = 0
          } 
            var x = matrix.x+(mouseOffset.x-LytePopup.xPos),
        ...
        /*------------------------------ MEASURE STARTS --------------------------*/
         $L.fastdom.measure(function() {
            var modalEle = this.actualModalDiv,
                ...
                if(modalEle.offsetParent && (modalEle.offsetParent.tagName === 'BODY')){
                    correctedTop = 0;
                }
            if(this.renderSidewise){
    ...
    calculateForSidewiseRender : function(){
        if(LytePopup.components.length > 1){
            ...
            if( parentModalId != '') {
                ...
                var curModalIndexInArray = LytePopup.components.indexOf(this);
				if((curModalIndexInArray > 0) && (LytePopup.components[curModalIndexInArray - 1].$node.tagName === "LYTE-MODAL")){
					prevModal = LytePopup.components[curModalIndexInArray - 1];
				}
            }
    ...
    closeModal : function(){
        ...
        _lyteUiUtils.dispatchEvent('lyteModalClosed' , this.actualModalDiv)
    ...
    onBeforeShowHandling : function(){
        ...
        if(result === undefined || result){
            ...
            this.$node.resetPosition = function(){
              LytePopup.x = this.getData('beforeDragPosition').xPos
