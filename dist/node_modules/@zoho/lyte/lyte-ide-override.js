...
_LC.core._constructor.prototype.updateForHelper = function(node, options, contextSwitchInfo, establishBindings, staticTempArr) {
    if(options){
    ...
    var ret =  _LC.core.updateForHelper.call(this, node, options, contextSwitchInfo, establishBindings, staticTempArr)
...
_LC.core._constructor.prototype.updateForInHelper = function(node, options, contextSwitchInfo, establishBindings) {
    if(options){
    ...
    var ret = _LC.core.updateForInHelper.call(this,node, options, contextSwitchInfo, establishBindings);
    if(node.tagName !== "TEMPLATE") {
        node  = node._origTemplate;
    }
    node._placeHolder.__lytehelper = node._placeHolder._helper;
    ...
    if(_LyteComponent.registeredComponents[componentName]) {
        ...
        if(comp.activeInstances > 0) {
            ...
            if(Lyte.Compile.getDynamicNodes) {
                var returnVal = Lyte.Compile.getDynamicNodes(componentName, undefined, template);
                if(returnVal.errors){
...
_LC.core._constructor._registerComponent = function(a,b){
    if(_LC._reRegisteredComponents.indexOf(a) != -1){
        this._template = JSON.parse(b._template);
    }
    _LC.core._registerComponent.call(this,a,b);
}
_LC.registerComponent = function(componentName, definition, options){
    var def = Lyte.deepCopyObject(definition);
    var originalOpt;
    if(options && options.mixins){
        originalOpt = Lyte.deepCopyObject(options);
    }
    _LC.core.registerComponent.call(this,componentName, definition, options);
